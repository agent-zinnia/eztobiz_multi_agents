...
class DualAgentSystem:
    ...
    async def answer_generated_question(self, generated_question: str, existing_thread_id: str) -> Dict[str, Any]:
        """Run the math agent with the generated question using the existing thread (reusing the same math agent)
        
        Args:
            generated_question: The question generated by the question agent
            existing_thread_id: The thread ID from the first math agent to reuse
        ...
        try:
            ...
            # Run the math agent with the generated question on the same thread
            run = await self.client.runs.create(
            ...
            # Wait for completion
            await self.client.runs.join(thread_id=existing_thread_id, run_id=run["run_id"])
            
            # Get the final state/messages
            state = await self.client.threads.get_state(thread_id=existing_thread_id)
    ...
    async def run_dual_agent_workflow(self, user_query: str) -> Dict[str, Any]:
        """Run the complete dual agent workflow: Math -> Question
        
        Args:
            user_query: The user's original question
            
        Returns:
            Dictionary containing results from both steps
        """
        ...
        # Step 3: Run the math agent again with the generated question (reusing the same thread)
        print("🔢 Step 3: Reusing math agent to answer the generated question...")
        answer_result = await self.answer_generated_question(generated_question, first_result["thread_id"])
